            ...
            <section className="bg-[#FAFAFA] py-20">
                <div className="container">
                    <div className="flex flex-col items-center gap-4">
                        <h2 className="text-[36px] leading-11 font-semibold text-center">The dot.it difference</h2>
                        <p className="text-lg leading-7 text-center">Not just doing things different but doing them better and smarter for every user.</p>
                    </div>
                    <div className="mt-15 grid grid-cols-[482px_1fr] items-center gap-18">
                        <div className='flex flex-col items-start gap-6'>
                            {Accordians.map((accordian, index) => (
                                <div key={index} className={`w-full py-3 px-5 rounded-3xl border cursor-pointer ${open === index ? "bg-[#F3F8FF] border-[#D9E7F7]" : "border-[#E9EAEB]"}  `}>
                                    <button onClick={() => handleToggle(index)} className='w-full text-start cursor-pointer text-lg leading-7.5 font-medium'>{accordian.title}</button>
